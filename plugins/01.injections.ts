...
export default defineNuxtPlugin((nuxtApp) => {
    ...
    const isSameLocale = (to, from): boolean => {
        const froms = from.path.split('/').slice(1)
        const tos = to.path.split('/').slice(1)
        let r = false
        if (froms.length === tos.length && froms.length > 0 && froms[0] === tos[0]) r = true
        return r
    }
    ...
    const disabledTransition = (to, from): boolean => {
        ...
        if (
            isSameLocale(to, from) &&
            from.path.indexOf('/privacy/') > -1 &&
